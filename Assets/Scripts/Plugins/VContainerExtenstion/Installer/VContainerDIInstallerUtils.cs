...
namespace SotongStudio.VContainer
{
    public class VContainerDIInstallerUtils
    {
        ...
        public static IEnumerable<RegistrationBuilder> RegisterComponents(IContainerBuilder builder, IEnumerable<MonoBehaviour> objectsScripting)
        {
            List<RegistrationBuilder> result = new();
            foreach (var target in objectsScripting)
            {
                var type = target.GetType();
                var regisBuilder = builder.Register(type, Lifetime.Singleton);

                if (type.GetCustomAttributes<RegisterAsAttribute>() is IRegisterAs regisAttb)
                {
